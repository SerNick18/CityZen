...
public class RiapriSegnalazione extends HttpServlet {
     ...
     * @param req request che contiene l'id della
     *            segnalazione chiusa da riaprire
     * @param resp oggetto che contiene la risposta che la servlet
     * deve ritornare al cliente
     * @throws ServletException per errori di validazione dei
     * campi ed autorizzazioni di sicurezza
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (idSegnalazioneDaAprire != null) {
            ...
            try {
                idSegnalazione =
                        Integer.parseInt(req.getParameter("idSegnalazione"));
            } catch (Exception e) {
                throw new MyServletException("Errore nell'elaborazione "
            ...
            Segnalazione segnalazioneChiusa =
            ...
            ArrayList<SegnalazioneInterface> segnalazioni =
                    (ArrayList<SegnalazioneInterface>)
                     service.getSegnalazioneByCittadino(
                             cittadino.getCF(), offset);
            while (segnalazioni.size() > 0) {
                for (SegnalazioneInterface s: segnalazioni) {
                    //controllo se la segnalazione del cittadino
                    // ha stato "inoltrata"
                    //e se fa riferimento alla stessa segnalazione
                    // che si sta cercando di riaprire con
                    //questa servlet
                    if (s.getStato().equals("inoltrata")
                            && s.getRiaperta() != 0
                            && s.getRiaperta() == segnalazioneChiusa.getId()) {
                        throw new MyServletException("Hai già riaperto "
                                + "questa segnalazione!");
                    }
                }
                offset += 20;
            ...
            //creo una nuova segnalazione con alcuni
            // campi di quella chiusa ereditati
            ...
            if ((uploadImage(req).equals(""))) {
                throw new MyServletException("E' obbligatorio allegare "
                        + "una foto alla segnalazione. "
     ...
     * sul formato accettato (jpg, png, jpeg),
     ...
     * e per quanto riguarda la memorizzazione del file
     * (se non viene memorizzato)
     * @throws ServletException per gestire errori relativi al
    ...
    private String uploadImage(HttpServletRequest request)
        ...
        try {
            ...
            if (!file.exists()) {
                throw new MyServletException("Errore nel "
                        + "caricamento della foto!");
            }
        } catch (IOException e) {
            throw new MyServletException("Errore I/O "
                    + "nel caricamento della foto!");
        }
        return fileName;
    }
}
